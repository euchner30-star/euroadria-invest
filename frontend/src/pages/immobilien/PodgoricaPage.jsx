...
import { MapPin, TrendingUp, Building2, Plane, Landmark, Briefcase, ArrowRight, CheckCircle, Clock, Euro, FileText, Phone, Shield } from 'lucide-react';
...
const PodgoricaPage = () => {
  ...
  useEffect(() => { fetch(`${process.env.REACT_APP_BACKEND_URL||''}/api/settings/downloads`).then(r=>r.json()).then(d=>setExposeUrl(d.podgorica_expose_url||'')).catch(()=>{}); }, []);
  ...
  const investmentHighlights = [
    { icon: TrendingUp, label: 'Investment-Score', value: '90/100', description: en?'Diversified Potential':'Diversifiziertes Potenzial' },
    { icon: Euro, label: en?'Average Price':'Durchschnittspreis', value: '€1.500-3.000/m²', description: en?'Capital Level':'Hauptstadt-Niveau' },
    { icon: Clock, label: en?'Time Horizon':'Zeithorizont', value: en?'2-5 Years':'2-5 Jahre', description: en?'Short to medium-term':'Kurz- bis mittelfristig' },
    { icon: Building2, label: en?'Potential':'Potenzial', value: '+35-55%', description: en?'Expected Appreciation':'Erwartete Wertsteigerung' },
  ];
  ...
  const whyItems = [
    { icon: Plane, title: en?'International Airport':'Internationaler Flughafen', desc: en?'TGD airport with connections to all major European cities.':'Flughafen TGD mit Verbindungen zu allen europäischen Großstädten.' },
    { icon: Landmark, title: en?'Capital Functions':'Hauptstadt-Funktionen', desc: en?'Government, universities, hospitals: stable infrastructure demand.':'Regierung, Universitäten, Krankenhäuser: stabile Infrastruktur-Nachfrage.' },
    { icon: Briefcase, title: en?'Business Hub':'Business-Zentrum', desc: en?'Growing IT scene, international companies, co-working spaces.':'Wachsende IT-Szene, internationale Unternehmen, Co-Working Spaces.' },
  ];

  return (
    <div className="min-h-screen bg-white" data-testid="podgorica-page">
      <SEO title={en?"Real Estate Podgorica | Investment Montenegro":"Immobilien Podgorica | Investment Montenegro"} description={en?"Capital real estate in Podgorica, Montenegro's business hub. Investment score 90/100, €1,500-3,000/m², diversified appreciation potential.":"Hauptstadt-Immobilien in Podgorica, Montenegros Business-Hub. Investment-Score 90/100, €1.500-3.000/m², diversifiziertes Wertsteigerungspotenzial."} url="/immobilien/podgorica" />

      <header className="pt-28 pb-16 px-6 bg-gradient-to-b from-ea-light to-white"><div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-2 text-ea-gold text-sm font-semibold tracking-wider uppercase mb-4"><MapPin className="w-4 h-4"/><span>{en?'Montenegro • Capital':'Montenegro • Hauptstadt'}</span></div>
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-semibold text-ea-dark mb-6 leading-tight">{en?<>Podgorica: <span className="text-ea-gold">Capital</span><br/>& Business Hub</>:<>Podgorica: <span className="text-ea-gold">Hauptstadt</span><br/>& Business-Hub</>}</h1>
        <p className="text-ea-dark/70 text-lg md:text-xl max-w-3xl mb-8 leading-relaxed">{en?"Podgorica is Montenegro's capital and economic center. With an international airport, growing IT scene and the largest concentration of purchasing power, the capital offers stable rental yields and diversified appreciation potential.":"Podgorica ist Montenegros Hauptstadt und Wirtschaftszentrum. Mit internationalem Flughafen, wachsender IT-Szene und der größten Kaufkraftkonzentration bietet die Hauptstadt stabile Mietrenditen und diversifiziertes Aufwertungspotenzial."}</p>
        <div className="flex flex-wrap gap-4">
          {exposeUrl?<ExposeLeadGate exposeUrl={exposeUrl} exposeName="Podgorica Exposé" sourceId="podgorica_expose" buttonText={en?"Download Exposé":"Exposé herunterladen"} buttonClass="px-6 py-3 bg-ea-gold text-ea-dark font-semibold rounded-lg hover:bg-ea-gold/90 transition-all flex items-center gap-2"/>:<button onClick={()=>setShowContactForm(true)} className="px-6 py-3 bg-ea-gold text-ea-dark font-semibold rounded-lg hover:bg-ea-gold/90 transition-all flex items-center gap-2" data-testid="podgorica-contact-cta"><FileText className="w-5 h-5"/>{en?'Request Exposé':'Exposé anfordern'}</button>}
      ...
      <section className="py-16 px-6 bg-ea-light"><div className="max-w-7xl mx-auto"><div className="grid lg:grid-cols-2 gap-12 items-center"><div><h2 className="text-3xl md:text-4xl font-semibold text-ea-dark mb-6">{en?<>Why <span className="text-ea-gold">Podgorica</span>?</>:<>Warum <span className="text-ea-gold">Podgorica</span>?</>}</h2><div className="space-y-4 mb-8">{whyItems.map((item,idx)=>(<div key={idx} className="flex items-start gap-4"><div className="w-10 h-10 bg-ea-gold/10 rounded-lg flex items-center justify-center flex-shrink-0"><item.icon className="w-5 h-5 text-ea-gold"/></div><div><h3 className="font-semibold text-ea-dark mb-1">{item.title}</h3><p className="text-ea-dark/70 text-sm">{item.desc}</p></div></div>))}</div>
        <div className="bg-white border border-ea-gold/20 rounded-xl p-6"><p className="text-ea-gold font-semibold text-sm mb-2">{en?'CAPITAL ANALYSIS':'HAUPTSTADT-ANALYSE'}</p><blockquote className="text-ea-dark/80 italic">{en?'"Podgorica combines the stability of a capital with the dynamics of a growing market. The city is the safest entry point for Montenegro newcomers."':'"Podgorica vereint die Stabilität einer Hauptstadt mit der Dynamik eines wachsenden Marktes. Die Stadt ist der sicherste Einstiegspunkt für Montenegro-Neulinge."'}</blockquote><p className="text-ea-dark/50 text-sm mt-3">EuroAdria Corporate Solutions Research Team</p></div></div>
      ...
      {showContactForm&&(<div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl p-8 max-w-lg w-full max-h-[90vh] overflow-auto shadow-2xl"><div className="flex items-center gap-3 mb-6"><div className="w-12 h-12 bg-ea-gold/10 rounded-xl flex items-center justify-center"><Landmark className="w-6 h-6 text-ea-gold"/></div><div><h3 className="text-xl font-bold text-ea-dark">Podgorica Exposé</h3><p className="text-ea-dark/60 text-sm">{en?'Capital Investment Analysis':'Hauptstadt-Investment-Analyse'}</p></div></div>
