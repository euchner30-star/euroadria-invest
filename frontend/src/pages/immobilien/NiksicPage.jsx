...
const NiksicPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white" data-testid="niksic-page">
      ...
      <section className="py-16 px-6 bg-white" id="apartments"><div className="max-w-7xl mx-auto"><div className="text-center mb-12"><h2 className="text-3xl md:text-4xl font-semibold text-ea-dark mb-4">{en?<>Available <span className="text-ea-gold">Properties</span></>:<>Verfügbare <span className="text-ea-gold">Immobilien</span></>}</h2><p className="text-ea-dark/70 max-w-2xl mx-auto">{en?'Commercial properties, apartments and development land. Contact us for current off-market offers.':'Gewerbeimmobilien, Apartments und Bauland. Kontaktieren Sie uns für aktuelle Off-Market Angebote.'}</p></div><LocationProperties location="Nikšić" en={en} /><div className="text-center"><button onClick={()=>setShowContactForm(true)} className="px-8 py-4 bg-ea-gold text-ea-dark font-semibold rounded-xl hover:bg-ea-gold/90 transition-all flex items-center gap-2 mx-auto"><Phone className="w-5 h-5"/>{en?'Register for Offers':'Für Angebote registrieren'}</button></div></div></section>
