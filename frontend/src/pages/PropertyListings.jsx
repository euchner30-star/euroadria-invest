...
import { MapPin, Maximize, BedDouble, Bath, Filter, Search, ChevronLeft, ChevronRight, Download, Phone, MessageSquare, X, Building2, Share2, Copy, Check } from 'lucide-react';
...
export function PropertyDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-white" data-testid="property-detail-page">
      <Helmet>
        <title>{`${property.title || 'Property'} – EuroAdria`}</title>
        <meta property="og:title" content={property.title || ''} />
        <meta property="og:description" content={`${property.location || ''} · ${property.property_type || ''} · ${priceText}`} />
        <meta property="og:image" content={ogImage} />
        <meta property="og:url" content={shareUrl} />
        <meta property="og:type" content="website" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={property.title || ''} />
        <meta name="twitter:description" content={`${property.location || ''} · ${priceText}`} />
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between gap-4 mb-2">
              <div>
                ...
                <h1 className="text-2xl sm:text-3xl font-bold text-[#04151F]">{property.title}</h1>
              </div>
              <div className="relative">
                <button onClick={() => setShowShare(!showShare)} className="flex items-center gap-2 px-4 py-2 bg-[#04151F]/5 hover:bg-[#04151F]/10 rounded-lg text-sm text-[#04151F]/70 transition-all shrink-0" data-testid="share-button">
                  <Share2 className="w-4 h-4" /> Share
                </button>
                {showShare && (
                  <div className="absolute right-0 top-12 bg-white rounded-xl shadow-lg border border-gray-100 p-2 w-56 z-50" data-testid="share-menu">
                    <button onClick={copyLink} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-sm text-[#04151F] transition-all">
                      {copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                      {copied ? 'Copied!' : 'Copy Link'}
                    </button>
                    <a href={`https://wa.me/?text=${encodeURIComponent(property.title + ' – ' + shareUrl)}`} target="_blank" rel="noopener noreferrer" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-sm text-[#04151F] transition-all">
                      <MessageSquare className="w-4 h-4 text-[#25D366]" /> WhatsApp
                    </a>
                    <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-sm text-[#04151F] transition-all">
                    ...
                    <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(property.title)}&url=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-sm text-[#04151F] transition-all">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                      X / Twitter
                    </a>
                    <a href={`mailto:?subject=${encodeURIComponent(property.title + ' – EuroAdria')}&body=${encodeURIComponent(property.title + '\n' + property.location + '\n\n' + shareUrl)}`} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-sm text-[#04151F] transition-all">
      ...
      <div className="bg-[#04151F] pt-24 pb-16 px-4">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-3xl mx-auto">
            <select value={locationFilter} onChange={e => setLocationFilter(e.target.value)} className="appearance-none bg-white/10 border border-white/20 rounded-lg pl-4 pr-8 py-2.5 text-white text-sm focus:outline-none focus:border-[#C8A96A] bg-[url('data:image/svg+xml;charset=UTF-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2212%22%20height%3D%2212%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22white%22%20stroke-width%3D%222%22%3E%3Cpath%20d%3D%22M6%209l6%206%206-6%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px] bg-[right_12px_center] bg-no-repeat cursor-pointer" data-testid="filter-location">
            ...
            <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="appearance-none bg-white/10 border border-white/20 rounded-lg pl-4 pr-8 py-2.5 text-white text-sm focus:outline-none focus:border-[#C8A96A] bg-[url('data:image/svg+xml;charset=UTF-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2212%22%20height%3D%2212%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22white%22%20stroke-width%3D%222%22%3E%3Cpath%20d%3D%22M6%209l6%206%206-6%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px] bg-[right_12px_center] bg-no-repeat cursor-pointer" data-testid="filter-type">
              {PROPERTY_TYPES.map(t => <option key={t} value={t} style={{ color: '#333' }}>{t === 'All' ? 'All Types' : t}</option>)}
            </select>
            <select value={priceRange} onChange={e => setPriceRange(e.target.value)} className="appearance-none bg-white/10 border border-white/20 rounded-lg pl-4 pr-8 py-2.5 text-white text-sm focus:outline-none focus:border-[#C8A96A] bg-[url('data:image/svg+xml;charset=UTF-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2212%22%20height%3D%2212%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22white%22%20stroke-width%3D%222%22%3E%3Cpath%20d%3D%22M6%209l6%206%206-6%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px] bg-[right_12px_center] bg-no-repeat cursor-pointer" data-testid="filter-price">
